...
//                    此时停止线程就可以了
            ...
            case REPET_INTERVAL:
                if (contentWidth <= (-xLocation)) {
                    //也就是说文字已经到头了
                    if (onContentBound != null) {
                        onContentBound.onBound();
                    }
                    xLocation = getWidth();
                }
                break;
...
//有兴趣的朋友可以自己完善，在现有的基础之上，静默追加新的 公告
    ...
    public interface OnContentBound {
        void onBound();
    }

    private OnContentBound onContentBound;


    public void setOnContentBound(OnContentBound onContentBound) {
        this.onContentBound = onContentBound;
    }
}