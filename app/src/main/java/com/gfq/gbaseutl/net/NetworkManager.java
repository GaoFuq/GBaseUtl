...
//                        LogManager.i("AppNetworkMgr", "网络连接类型为：TYPE_WIFI, 网络连接状态CONNECTED成功！");
    ...
    public static boolean setWifi(Context context, boolean enable)
        ...
        if (isWifiOpen(context) != enable) {
            ((WifiManager) context.getApplicationContext().getSystemService(Context.WIFI_SERVICE)).setWifiEnabled(enable);
        }
        return true;
    }
